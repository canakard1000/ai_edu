...
export default function App() {
  ...
  return <main><div className="app-bar"><button onClick={() => go('home')} aria-label="딱모여 홈">딱모여</button></div>{toast ? <div className="toast" role="status">{toast}<button onClick={() => setToast('')}>닫기</button></div> : null}<div className="content">{screen === 'home' && renderHome()}{screen === 'groups' && renderGroups()}{screen === 'detail' && renderDetail()}{screen === 'create' && renderCreate()}{screen === 'profile' && renderProfile()}{screen === 'help' && renderHelp()}</div><BannerAd /><nav><button className={screen === 'home' ? 'active' : ''} onClick={() => go('home')}>홈</button><button className={screen === 'groups' || screen === 'detail' ? 'active' : ''} onClick={() => go('groups')}>모임</button><button onClick={() => go('create')}>만들기</button><button onClick={onShare}>공유</button><button className={screen === 'profile' ? 'active' : ''} onClick={() => go('profile')}>내정보</button><button className={screen === 'help' ? 'active' : ''} onClick={() => go('help')}>도움</button></nav></main>;
}
